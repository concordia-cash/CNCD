...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("Concordia client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("Concordia"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("Concordia"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
